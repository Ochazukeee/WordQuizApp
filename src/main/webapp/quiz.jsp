...
<body>
    <div class="container">
			...
			<c:when test="${not empty currentWord}">
                <div class="question-section">
					...
					<c:choose>
							...
							<div class="action-buttons">
								<c:choose>
									<c:when test="${currentQuestion < 10}">
										<a href="WordQuizAppController?action=next" class="btn btn-primary">Next Question</a>
									</c:when>
									<c:otherwise>
										<a href="WordQuizAppController?action=result" class="btn btn-primary">View Results</a>
									</c:otherwise>
								</c:choose>
							</div>
						</c:when>
						...
						<%-- Not answerd yet--%> 
						<c:otherwise>
							<form method="post" action="WordQuizAppController">
								<input type="hidden" name="action" value="check">
								<div class="input-section">
									<input type="text" name="answer" id="answer" placeholder="Enter in English" required>
		...
		<%-- in case currentWord does not exist--%> 
			<c:otherwise>
  				<div class="error-section">
					<p>Failed to load question.</p>
					<a href="WordQuizAppController?action=start" class="btn btn-primary">Start from Beginning</a>
				</div>
			</c:otherwise>
		</c:choose>
